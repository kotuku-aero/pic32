...
/* XC32-2550: Return the free space left on the heap.
 * This allows user to call malloc after manually called
 * sbrk().
 * However, if the user calls malloc and then attempts to
 * call sbrk, the call to sbrk will fail because malloc
 * will "eat" all the heap space.
 */
int _get_heap_left_space()
{
    if (!curbrk)
        _sbrk_init();

    return (_maxbrk - (void *)curbrk);
}
...
_sbrk (int n)
{
    ...
    if (!curbrk) {
        ...
        /* if curbrk is still null after init, it means init failed */
        if (!curbrk)
	        return (void *)-1;
    }

    if (n < 0) {
        if ((size_t) (curbrk - _minbrk) < (size_t) (-n))
            return (void *) -1;
    } else {
        if ((size_t) (_maxbrk - curbrk) < (size_t) n)
            return (void *) -1;
    }
